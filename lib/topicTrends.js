...
// The home page's "Rising in AI" and "Rising in Cyber Security" sections: which subject
// TAGS are gaining share, per topic, over 24h / 7d / 30d.
//
...
function rankTopic(topic, tally, baseline, topicsByTag, { minMentions, limit }) {
  ...
  for (const [tag, { now: nowN, prev: prevN }] of counts) {
    if (nowN === 0 || !topicsByTag.get(tag).has(topic)) {
      continue;
    }
...
function aggregateTopicTrends(articleRows, tagTopicRows, { now = Date.now(), limit = DEFAULT_TREND_LIMIT } = {}) {
  ...
  TREND_WINDOWS.forEach((window, index) => {
    ...
    windows[window.key] = Object.fromEntries(TREND_TOPICS.map((topic) => [
      topic,
