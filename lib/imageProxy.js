...
const { rasterizeSvg } = require('./svgRaster');
...
function requestPinned(url, options, addresses) {
  ...
  const lookup = (hostname, lookupOptions, callback) => {
    ...
    const eligible = family ? addresses.filter((record) => record.family === family) : addresses;
...
async function fetchWithValidatedRedirects(
  ...
  options,
  redirectCount = 0,
  fetchImpl = requestPinned,
) {
  const addresses = await validateUrlTarget(url);

  // The connector receives the exact records validated above. It never performs a
  // second DNS lookup, closing the validation-to-connection rebinding window.
  const response = await fetchImpl(url, options, addresses);
  ...
  return fetchWithValidatedRedirects(redirectUrl, options, redirectCount + 1, fetchImpl);
}
...
async function proxyImage(req, res, { fetchImpl = requestPinned } = {}) {
  ...
  try {
    ...
    // Keep the same AbortSignal live while consuming the body. A server that sends
    // headers promptly and then stalls must not hold the function open indefinitely.
  ...
  let contentType = detectImageContentType(buffer, upstreamResponse.headers.get('content-type') || '');
  ...
  // SVG never leaves as SVG (it can carry script); it is rendered to PNG instead.
  if (contentType === 'image/svg+xml') {
    try {
      buffer = await rasterizeSvg(buffer);
      contentType = 'image/png';
    } catch (err) {
      logMetric('image_proxy.error', 1, { reason: 'svg_render_failed', requestId: req.requestId });
      return res.status(415).json({ error: 'SVG image could not be rendered' });
    }
  }
