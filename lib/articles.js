...
//
// Raised 300 -> 600 so more of the archive is searchable instantly, without an
// escalation to /api/search. The binding cost is payload, not compute: measured
// 2026-09-06, 300 rows serialise to 74 KB gzipped and 600 to roughly 145 KB, while
// the database time is flat (~1.65 s at every limit — see the excerpt note on
// PUBLIC_ARTICLES_RELATION below) and client-side filtering stays under 0.15 ms
// per keystroke even at 1000 rows. Keep API_ARTICLE_LIMIT in react-app/src/App.jsx
// and the clamp in sql/create-search-articles-function.sql in step with this: the
// client asking for more than this cap is a 400, not a truncation.
const MAX_LIMIT = 600;
