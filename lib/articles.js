...
// Each row's timestamps are parsed once up front rather than inside the comparator:
// getArticleTimestamp is regexes plus up to two Date.parse calls, and a comparator
// runs it ~n log n times -- tens of thousands of parses for a full working set.
function sortArticlesNewestFirst(articles) {
  return articles
    .map((article) => {
      // A page that carries no date anywhere in its markup (anthropic.com's threat
      // intelligence reports) sorts by when it was scraped instead. Without the
      // fallback it sorts as 0 and sinks below the whole archive, which reads as a
      // bug in a newest-first list. The fetch is an upper bound on the real date.
      const fetched = parseTimestamp(article.fetched_at);
      return { article, published: getArticleTimestamp(article) || fetched, fetched };
    })
    .sort((a, b) => (b.published - a.published) || (b.fetched - a.fetched))
    .map(({ article }) => article);
}

function buildFetchArticlesWhere({ site, notSite, topic, tags, notTags } = {}) {
  const clauses = [];
  const params = [];
  const normalizedSites = normalizeMultiFilter(site, 'site', MAX_SITE_LENGTH);
  const excludedSites = normalizeMultiFilter(notSite, 'not_site', MAX_SITE_LENGTH);
  const normalizedTopics = normalizeMultiFilter(topic, 'topic', MAX_TOPIC_LENGTH);

  // not_site keeps whole sources off a page that has its own home elsewhere: the
  // front page drops the exploit feeds, which /threats shows instead.
  if (excludedSites) {
    params.push(excludedSites);
    clauses.push(`site <> ALL($${params.length}::text[])`);
  }
...
// The article list is two queries, and the split is the whole performance story.
// public_articles computes `excerpt` with a regexp_replace over body_text, and
// Postgres only evaluates a view column that the query references. Measured on
// staging 2026-09-13 over 3,839 articles: the full column list for the working set
// took 1910 ms; the narrow index columns below took 36 ms. So the working set is
// selected, filtered, sorted and faceted on the narrow columns, and the display
// columns (excerpt included) are read only for the page actually being returned.
//
// Stage 1: the index. Everything ordering, paging and faceting need, nothing else.
function buildFetchArticlesQuery({ site, notSite, topic, tags, notTags } = {}) {
  const { where, params } = buildFetchArticlesWhere({ site, notSite, topic, tags, notTags });
...
// Stage 2: the display columns for one page. `url = ANY(...)` is built on texteq,
// which is leakproof, so the security barrier lets it reach articles_pkey (verified
// with EXPLAIN on staging) and the excerpt is built for the page's rows only. url is
// not unique on its own -- the key is (site, url) -- so callers match rows back on
// both.
const ARTICLE_PAGE_SQL = `
  SELECT
    url,
    site,
    topic,
...
// The sorted index is held briefly per warm function instance, so the home page's
// background page requests reuse the one stage-1 query instead of each repeating it.
// The TTL matches the endpoint's s-maxage: nothing here is staler than the edge cache
// already allows.
const ARTICLE_INDEX_TTL_MS = 60 * 1000;
const ARTICLE_INDEX_CACHE_SIZE = 20;
const articleIndexCache = new Map();

async function fetchArticleIndex(filters) {
  const { text, params } = buildFetchArticlesQuery(filters);
  const cacheKey = JSON.stringify(params) + text;
  const cached = articleIndexCache.get(cacheKey);
  ...
  const rows = sortArticlesNewestFirst(result.rows);

  articleIndexCache.delete(cacheKey);
  articleIndexCache.set(cacheKey, { rows, expiresAt: Date.now() + ARTICLE_INDEX_TTL_MS });
  while (articleIndexCache.size > ARTICLE_INDEX_CACHE_SIZE) {
    articleIndexCache.delete(articleIndexCache.keys().next().value);
  }

  return rows;
}

async function fetchArticlePage(indexRows) {
  if (indexRows.length === 0) {
    return [];
  }

  const urls = [...new Set(indexRows.map((row) => row.url))];
  const result = await query(ARTICLE_PAGE_SQL, [urls]);
  const byKey = new Map(result.rows.map((row) => [articleKey(row), row]));

  // Index order, not the page query's order. A row that disappeared between the two
  // queries (withheld for review, say) is simply dropped from the page.
  return indexRows.map((row) => byKey.get(articleKey(row))).filter(Boolean);
}
...
// One page of the working set, plus what a client needs to fetch the rest:
// `total` rows in the working set and `next_offset` (null on the last page).
//
// Facets are tallied over the WHOLE working set, not the page, and only on the first
// page (later pages carry null). The filter panel shows them from first paint, before
// the remaining pages have arrived, so a page-sized tally would show counts that the
// full edition then contradicts.
const isLeadRow = (row) => (
  Array.isArray(row.lead_topics) ? row.lead_topics.length > 0 : Boolean(row.is_lead)
);

async function fetchArticles({ site, notSite, topic, tags, notTags, limit, offset } = {}) {
  const pagination = normalizePagination({ limit, offset });
  const index = await fetchArticleIndex({ site, notSite, topic, tags, notTags });
  const slice = index.slice(pagination.offset, pagination.offset + pagination.limit);
  // The first page also carries every lead further down the index, so the front
  // page's lead carousel is complete at first paint rather than gaining a story
  // when a background page lands. Paging is unchanged: the lead arrives again in its
  // own page and the client dedupes it on (site, url).
  const pageRows = pagination.offset === 0
    ? [...slice, ...index.slice(slice.length).filter(isLeadRow)]
    : slice;
  const items = await fetchArticlePage(pageRows);
  const nextOffset = pagination.offset + slice.length;

  return {
    items,
    facets: pagination.offset === 0 ? buildFacets(index) : null,
    total: index.length,
    offset: pagination.offset,
    limit: pagination.limit,
    next_offset: nextOffset < index.length ? nextOffset : null,
  };
}
...
async function countArticles({ site, notSite, topic, tags, notTags } = {}) {
  const { where, params } = buildFetchArticlesWhere({ site, notSite, topic, tags, notTags });
  const result = await query(`
